...
| **三级标题** | `### 节标题` | 棕色文字 + 橘色竖线 + 居中横线 | 概览分节或正文小标题 |
| **段落文本** | `普通文本` | 标准段落样式 | 支持内联代码样式 |
| **无序列表** | ``- `#1` 列表项`` | 橘色编号标签 | 编号由 Markdown 提供 |
...
0.13.0 为输出中每个显式 HTML 元素添加 `data-no-dark`，同时保护容器背景和后代文字。
原有颜色、字体、间距及 HTML 内容保持不变；深色模式下正文仍保持原来的浅色外观，
不是自动切换深色主题。微信官方说明该属性不递归作用于后代，因此不能只添加在根容器上。

实现使用 HTML 解析器定位源码并插入属性，不重新序列化 HTML；已有保色属性不会重复添加。
浏览器自动补出的无样式节点（如 `tbody`）无需额外处理。此属性用于控制微信正文算法，
不保证覆盖手机系统另行施加的强制变色。

验证：`npm run build && node --test tests/preserve-colors.test.mjs`。

0.13.1 补充：列表文字节点显式使用原黑色；未自定义颜色的“查看原文”链接固定为微信浅色链接色 `#576b95`，避免 `--weui-LINK` 随主题变浅。背景及其他配色不变。
